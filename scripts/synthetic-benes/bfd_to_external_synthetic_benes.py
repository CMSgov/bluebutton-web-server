...
parser.add_argument('--bedap', '-bo', help='The name of the bedap output csv file', type=str)
parser.add_argument('--sls', '-so', help='The name of the sls output csv file', type=str)
parser.add_argument(
    '--benecount',
    '-bc',
...
if not args.benecount:
    raise Exception('benecount is required, without it the correct sls username/passwords cannot be created')
...
STARTING_NEW_BEDAP_BENE_KEY = 999000001 + (INITIAL_SYNTHETIC_BENE_COUNT - BASELINE_SYNTHETIC_BENE_COUNT)
...
def random_date():
    start_date = datetime.datetime.now() - datetime.timedelta(days=DEFAULT_MAX_YEARS_AGO_FOR_EFFECTIVE_DATE * 365)
...
def convert_to_bedap_bene(bfd_bene, bene_key, effective_date=DEFAULT_EFFECTIVE_DATE):
    ...
    for key in BEDAP_FIELD_MAPPING:
        if isinstance(BEDAP_FIELD_MAPPING[key], str):
            ...
            bedap_bene[BEDAP_FIELD_MAPPING[key]['name']] = BEDAP_FIELD_MAPPING[key]['value'](bfd_bene[key])
...
def convert_to_sls_bene(bfd_bene, bene_count, effective_date=DEFAULT_EFFECTIVE_DATE):
    ...
    for key in SLS_FIELD_MAPPING:
        if isinstance(SLS_FIELD_MAPPING[key], str):
            ...
            sls_bene[SLS_FIELD_MAPPING[key]['name']] = SLS_FIELD_MAPPING[key]['value'](bfd_bene[key])
...
with open(INPUT_FILE_NAME, newline='') as input_csvfile:
    ...
    with open(BEDAP_OUTPUT_FILE_NAME, 'w+', newline='') as bedap_output_csvfile:
        ...
        with open(SLS_OUTPUT_FILE_NAME, 'w+', newline='') as sls_output_csvfile:
            ...
            for row in reader:
                bedap_bene = convert_to_bedap_bene(row, str(new_bene_key), effective_date=effective_date)
                sls_bene = convert_to_sls_bene(row, str(new_bene_count), effective_date=effective_date)
