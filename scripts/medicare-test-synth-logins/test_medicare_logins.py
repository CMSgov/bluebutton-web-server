...
parser = argparse.ArgumentParser(description='Utility to test a medicare.gov login using Selenium')
parser.add_argument(
    '--url-base',
    '-u',
...
SLEEP_BETWEEN_ACTIONS = args.sleep_between_actions if args.sleep_between_actions else None
...
CONNECTION_ISSUE_SLEEP = args.connection_issue_sleep if args.connection_issue_sleep else None
MAX_CONNECTION_ISSUE_COUNT = args.max_connection_issue_count if args.max_connection_issue_count else None
...
for n in range(BEGIN_NUM, END_NUM + 1):
    ...
    for t in range(0, MAX_RETRY):
        print(f'# TESTING({t}): n={n} username={username}')
