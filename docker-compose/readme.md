...
```
To change the number of refresh tokens, archived tokens, or archived access grants per user, use the `-r` flag to modify refresh token count and the `-t` flag for archived token count and the `-g` flag for archived access grants. These are also listed when specifying `--help` on the command.
