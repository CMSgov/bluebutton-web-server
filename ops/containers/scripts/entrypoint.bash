...
# FIXME: This should only run when running locally, so we can talk to S3.
echo "🔵 running socat localhost:9090 -> docker.internal:9090"
# If socat isn't installed, just skip this. We only want to run it locally.
socat TCP-LISTEN:9090,fork,reuseaddr TCP:host.docker.internal:9090 || true &
...
printf "BOTONBIN: %s" "${BOTONBIN}"

mkdir -p ${NGINX_TMP}/tmp
cat ${HOME}/bb/ops/containers/bb-web-api/files/nginx.conf.in | ${BOTONBIN}/envsubst  > ${NGINX_TMP}/nginx.conf
