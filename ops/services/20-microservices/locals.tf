# terraform/services/20-microservices/locals.tf
# Configuration from SSM Parameter Store with defaults for initial deployment

locals {
  # Get values from platform object
  service         = "microservices"
  workspace       = local.platform.env
  app_prefix      = local.platform.app
  cluster_name    = data.aws_ecs_cluster.main.cluster_name
  private_subnets = local.platform.private_subnet_ids
  public_subnets  = local.platform.public_subnet_ids
  region          = local.platform.primary_region.id
  account_id      = local.platform.account_id

  # Build service configuration from SSM JSON + overrides
  ...
  # VPC ID and AZs from platform module (not SSM)
  vpc_id = local.platform.vpc_id
  azs    = [for s in values(local.platform.private_subnets) : s.availability_zone]

  common_tags = local.platform.default_tags
