# This file was created as part of BB2-4438. We wanted to keep the sourcing of env vars in deployed environments the same as it
# was before we began the migration to Fargate, to reduce complexity and the chances of problems while we still deploy that way.
...
# security
SECRET_KEY = env('DJANGO_SECRET_KEY', 'FAKE_SECRET_KEY_YOU_MUST_SET_DJANGO_SECRET_KEY_VAR')
...
DATABASES = {
    'default': dj_database_url.config(default=env('DATABASES_CUSTOM', 'sqlite:///{}/db.sqlite3'.format(BASE_DIR))),
}
...
APPLICATION_TITLE = env('DJANGO_APPLICATION_TITLE', 'Blue Button API')
...
APPLICATION_RESEARCH_STUDY_ENDED_MESG = (
    'Application end date passed. Contact us at BlueButtonAPI@cms.hhs.gov if you need to refresh Medicare data.'
)
...
FHIR_POST_SEARCH_PARAM_IDENTIFIER_MBI_HASH = 'https://bluebutton.cms.gov/resources/identifier/mbi-hash'

FHIR_POST_SEARCH_PARAM_IDENTIFIER_HICN_HASH = 'https://bluebutton.cms.gov/resources/identifier/hicn-hash'
...
FHIR_PATIENT_SEARCH_PARAM_IDENTIFIER_MBI = 'http://hl7.org/fhir/sid/us-mbi'
...
MEDICARE_SLSX_REDIRECT_URI = env('DJANGO_MEDICARE_SLSX_REDIRECT_URI', 'http://localhost:8000/mymedicare/sls-callback')

MEDICARE_SLSX_LOGIN_URI = env(
    'DJANGO_MEDICARE_SLSX_LOGIN_URI',
    'https://test.medicare.gov/sso/authorize?client_id=bb2api',
)
SLSX_HEALTH_CHECK_ENDPOINT = env('DJANGO_SLSX_HEALTH_CHECK_ENDPOINT', 'https://test.accounts.cms.gov/health')
SLSX_TOKEN_ENDPOINT = env('DJANGO_SLSX_TOKEN_ENDPOINT', 'https://test.medicare.gov/sso/session')
SLSX_SIGNOUT_ENDPOINT = env('DJANGO_SLSX_SIGNOUT_ENDPOINT', 'https://test.medicare.gov/sso/signout')
...
CLEAR_HIGHER_JWKS_URL = env(
    'DJANGO_CLEAR_HIGHER_JWKS_URL', 'https://verified.clearme.com/integrations/.well-known/jwks.json'
)
CLEAR_LOWER_JWKS_URL = env('DJANGO_CLEAR_LOWER_JWKS_URL', 'https://please-find-out-what-i.am')
IDME_HIGHER_JWKS_URL = env('DJANGO_IDME_HIGHER_JWKS_URL', 'https://api.id.me/oidc/.well-known/jwks')
IDME_LOWER_JWKS_URL = env('DJANGO_IDME_LOWER_JWKS_URL', 'https://api.idmelabs.com/oidc/.well-known/jwks')
