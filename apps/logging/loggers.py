...
def log_global_state_metrics_to_itslog(group_timestamp=None, report_flag=True):
    """
    For use in apps/logging/management/commands/log_global_metrics.py management command
    NOTE:  print statements are for output when run via Jenkins
    """
    ...
    for key in log_dict.keys():
        print("looping app: ", key, log_dict[key])
        val_to_post = log_dict[key]
        print("val to post: ", val_to_post, type(val_to_post))
        if isinstance(val_to_post, type(bool)) or isinstance(val_to_post, type(int)):
            val_to_post = str(val_to_post)
        result = ping_api([key], val_to_post)
    ...
    for app in applications:
        ...
        logger.info(log_dict, cls=DjangoJSONEncoder)
        print("THE DICT APP: ", log_dict)
        for key in log_dict.keys():
            tag = str(app.id) + '_' + key
            print("looping app: ", tag, log_dict[key])
            val_to_post = log_dict[key]
            print("val to post: ", val_to_post, type(val_to_post))
            if isinstance(val_to_post, type(bool)) or isinstance(val_to_post, type(int)):
                val_to_post = str(val_to_post)
                print("CONVERTED TO STR: ", val_to_post, type(val_to_post))
            result = ping_api([tag], val_to_post)
            print("RESULT: ", result.json())
