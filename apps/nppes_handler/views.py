...
def nppes_update(request):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            if response['code'] == 200:
                response['status'] = "OK"
                response['message'] = 'The NPPES update ' \
                                      'for %s ' \
                                      'succeeded.' % (nppes_write_response['number'])
            else:
                response['status'] = 'FAIL'
                response['message'] = 'The NPPES update for ' \
                                      '%s failed.' % (nppes_write_response['number'])
    ...
    response = OrderedDict()
    response['code'] = 200
    response['message'] = 'POST Provider JSON as the body of ' \
                          'the request to this URL.'
