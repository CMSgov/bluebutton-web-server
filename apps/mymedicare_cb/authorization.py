...
class OAuth2ConfigSLSx(object):
    ...
    def exchange_for_access_token(self, req_token, request):
        ...
        response = requests.post(
            ...
            hooks={'response': [response_hook_wrapper(sender=SLSxTokenResponse, request=request)]},
        )
    ...
    def get_user_info(self, request):
        ...
        self.validate_asserts(
            ...
            [
                (
                    data_user_response is None or data_user_response.get('id', None) is None,
                    MSG_SLS_RESP_MISSING_USERINFO_USERID,
                ),
                (
    ...
    def log_authn_success(self, request, extra):
        ...
        log_dict = {'type': 'Authentication:success', 'sub': self.user_id, 'user': None, 'path': path}
        log_dict.update(extra)
        logger.info(log_dict)
