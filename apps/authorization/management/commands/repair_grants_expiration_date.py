...
class Command(BaseCommand):
    help = 'Repair Data Access Grant expiration_date for an app, pass in the app name whose grants need repair.'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--turnondate',
            help='Datetime (UTC) when limit data access feature turned on for the app, '
            "format: '%%m/%%d/%%y %%H:%%M:%%S', e.g. '12/04/24 15:58:26'.",
        )
        parser.add_argument(
            '--range',
            help='Datetime range (UTC) the grants checked for processing should be created in, '
            'format: <begindate>-<enddate>, inclusive on both begin and end, '
            "date format: %%m/%%d/%%y %%H:%%M:%%S, e.g. '01/16/25 19:58:26-01/18/25 23:59:59'.",
        )
        parser.add_argument(
            '--dryrun', action='store_true', help='Dry run the command without making changes to the database.'
        )
    ...
    def handle(self, *args, **options):
        ...
        if 'THIRTEEN_MONTH' not in app.data_access_type:
            print(
                "Error: This command only applies to app with THIRTEEN_MONTH data access type, '{}' data access type: {}.".format(
                    app.name, app.data_access_type
                )
            )
            return False
        ...
        grants = DataAccessGrant.objects.filter(application=app.id).filter(
            created_at__range=(range_begin_date, range_end_date)
        )
        print(
            'process restriction: feature turned on date: {} and created_at in range: [{}, {}].'.format(
                turn_on_date, range_begin_date, range_end_date
            )
        )
        ...
        if grants.exists():
            ...
            if turn_on_date is not None:
                print(
                    'Processed grants: {}, created after feature turned on date: {}, created before turned on date: {}'.format(
                        processed, after_turn_on, before_turn_on
                    )
                )
            else:
...
def validate_parameters(options, dt_fmt):
    ...
    if len(r_value) == 2:
        ...
        print(
            "Error: Malformed --range value, expecting value like: '01/16/25 19:58:26-01/18/25 23:59:59', received: {}.".format(
                range_str
            )
        )
        result['params_are_valid'] = False
    ...
    if r_end_d > current_dt:
        print(
            'Error: --range end date must be a past time, range_end_date = {}, current date time ={}'.format(
                r_end_d, current_dt
            )
        )
        result['params_are_valid'] = False
    ...
    if r_begin_d > r_end_d:
        print(
            'Error: --range value must be range_begin_date <= range_end_date: receiving range_begin_date = {}, range_end_date = {}'.format(
                r_begin_d, r_end_d
            )
        )
        result['params_are_valid'] = False
    ...
    if turn_on_date_str is not None:
        ...
        if turn_on_d >= current_dt:
            print(
                'Error: --range end date must be a past time, turn_on_date = {}, current date time ={}'.format(
                    turn_on_d, current_dt
                )
            )
            result['params_are_valid'] = False
            return result
