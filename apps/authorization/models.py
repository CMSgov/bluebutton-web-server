...
def get_beneficiary_counts():
    ...
    def create_grant_queryset(version):
...
def get_beneficiary_grant_app_pair_counts():
    ...
    real_grant_queryset_v2 = real_grant_filter(version=Versions.V2)
    real_grant_queryset_v3 = real_grant_filter(version=Versions.V3)
    real_grant_queryset = real_grant_queryset_v2.union(real_grant_queryset_v3)

    # BB2-4166-TODO: currently only checking for v2
    def synthetic_grant_filter(version):
        field = f"beneficiary__crosswalk__fhir_id_v{version}"
        return grant_queryset.filter(
            Q(**{f"{field}__startswith": "-"})
            & ~Q(**{field: ""})
    ...
    synthetic_grant_queryset_v2 = synthetic_grant_filter(version=Versions.V2)
    synthetic_grant_queryset_v3 = synthetic_grant_filter(version=Versions.V3)
    ...
    def real_grant_archived_filter(version):
        field = f"beneficiary__crosswalk__fhir_id_v{version}"
        return grant_archived_queryset.filter(
            ~Q(**{f"{field}__startswith": "-"})
            & ~Q(**{field: ""})
            & Q(**{f"{field}__isnull": False})
        ).values("beneficiary", "application")
    real_grant_archived_queryset_v2 = real_grant_archived_filter(version=Versions.V2)
    real_grant_archived_queryset_v3 = real_grant_archived_filter(version=Versions.V3)
    ...
    # BB2-4166-TODO: this only checks v2! it should be |or| on v3 as well
    def synthetic_grant_archived_filter(version):
        field = f"beneficiary__crosswalk__fhir_id_v{version}"
        return grant_archived_queryset.filter(
            Q(**{f"{field}__startswith": "-"})
            & ~Q(**{field: ""})
    ...
    synthetic_grant_archived_queryset_v2 = synthetic_grant_archived_filter(version=Versions.V2)
    synthetic_grant_archived_queryset_v3 = synthetic_grant_archived_filter(version=Versions.V3)
