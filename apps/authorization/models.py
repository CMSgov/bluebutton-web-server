...
class DataAccessGrant(models.Model):
    beneficiary = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, editable=False)
    application = models.ForeignKey(oauth2_settings.APPLICATION_MODEL, on_delete=models.CASCADE, editable=False)
...
def get_grant_bene_counts(application=None):
    ...
    if application is None:
        ...
        counts_returned['synthetic_deduped'] = synthetic_grant_queryset.distinct().count()
    ...
    counts_returned['archived_synthetic_deduped'] = synthetic_archived_queryset.distinct().count()
    counts_returned['archived_deduped_elapsed'] = round(datetime.utcnow().timestamp() - start_time, 3)
    ...
    synthetic_union_queryset = synthetic_grant_queryset.union(synthetic_archived_queryset)
    ...
    counts_returned['grant_and_archived_real_deduped'] = real_union_queryset.count()
    counts_returned['grant_and_archived_synthetic_deduped'] = synthetic_union_queryset.count()
    counts_returned['grant_and_archived_deduped_elapsed'] = round(datetime.utcnow().timestamp() - start_time, 3)
...
def get_beneficiary_counts():
    ...
    counts_returned['synthetic_grant'] = synthetic_queryset.filter(Q(grant_count__gt=0)).count()

    # Count only if in grant archived
    counts_returned['total_grant_archived'] = queryset.filter(Q(grant_archived_count__gt=0)).count()
    counts_returned['real_grant_archived'] = real_queryset.filter(Q(grant_archived_count__gt=0)).count()
    ...
    """
    counts_returned['real_grant_to_apps_eq_1'] = real_queryset.filter(Q(grant_count=1)).count()
    counts_returned['synthetic_grant_to_apps_eq_1'] = synthetic_queryset.filter(Q(grant_count=1)).count()

    counts_returned['real_grant_to_apps_eq_2'] = real_queryset.filter(Q(grant_count=2)).count()
    counts_returned['synthetic_grant_to_apps_eq_2'] = synthetic_queryset.filter(Q(grant_count=2)).count()
    ...
    counts_returned['real_grant_to_apps_gt_13'] = real_queryset.filter(Q(grant_count__gt=13)).count()
    counts_returned['synthetic_grant_to_apps_gt_13'] = synthetic_queryset.filter(Q(grant_count__gt=13)).count()
...
def get_beneficiary_grant_app_pair_counts():
    ...
    # Setup base queryset
    grant_archived_queryset = ArchivedDataAccessGrant.objects.values('beneficiary', 'application')
    ...
    counts_returned['synthetic_grant_archived'] = synthetic_grant_archived_queryset.count()

    """
    Bene<->App pair differences
    """
    # Pairs in Grant but not in ArchivedGrant.
    counts_returned['grant_vs_archived_difference_total'] = grant_queryset.difference(grant_archived_queryset).count()
    counts_returned['real_grant_vs_archived_difference_total'] = real_grant_queryset.difference(
        real_grant_archived_queryset
    ).count()
    counts_returned['synthetic_grant_vs_archived_difference_total'] = synthetic_grant_queryset.difference(
        synthetic_grant_archived_queryset
    ).count()

    # Pairs in ArchivedGrant but not in Grant.
    counts_returned['archived_vs_grant_difference_total'] = grant_archived_queryset.difference(grant_queryset).count()
    counts_returned['real_archived_vs_grant_difference_total'] = real_grant_archived_queryset.difference(
        real_grant_queryset
    ).count()
    counts_returned['synthetic_archived_vs_grant_difference_total'] = synthetic_grant_archived_queryset.difference(
        synthetic_grant_queryset
    ).count()
