...
FILE: update_ids_with_unique.py
Created: 12/07/19 5:35 PM

Created by: '@ekivemark'

Import csv file and update ids with  unique ids
...
CONFIRM_STRING = "REPLACE_WITH_UNIQUE_IDS"

class Command(BaseCommand):
    help = 'Report usernames with SLS collisions'

    def add_arguments(self, parser):
        parser.add_argument('--guid_file', type=str, help='absolute path to input file containing guids and unique uuid')
    ...
    def handle(self, *args, **kwargs):
        ...
        if not guid_file:
            print("Please provide --guid_file {path/to/filename}\nguid_file is a csv of collided uuids with no field header")
            return
        ...
        line = ["id", "username", "short_guid", "short_guid_line", "user_date_joined", "user_last_login",
                ...
                "crosswalk_id", "crosswalk_fhir_id","crosswalk_hicn_hash", "crosswalk_date_created", "action"]
        ...
        ct = 1
        for g in guid_list:
            # guid list has two fields
            # {unique quid}, {short_guid}
            #
            ...
            print("User=%s new username=%s [%s %s] %s" % (u_username, new_i, u_first, u_last, cx_fhir))
            ...
            line = [u_id, u_username, i, ct, u_joined, u_login,
                    ...
                    cx_id, cx_fhir, cx_hash, cx_date, action]
            # append the line
            out_list.append(line)
