...
class ResetPasswordWhileAuthenticatedTestCase(TestCase):
    ...
    def test_password_change_complexity_and_min_age_validation(self):
        ...
        self.assertEqual(self.user.check_password('IchangedthePassword@123'), True)
        form_data = {
            'old_password': 'IchangedthePassword@123',
        ...
        StubDate.now = classmethod(lambda cls, timezone: datetime.now().replace(tzinfo=pytz.UTC))
        response = self.client.post(url, form_data, follow=True)
    ...
    def test_no_password_expire(self):
        ...
        self.assertContains(response, ('My Sandbox Apps'))
        self.assertContains(response, ('The Developer Sandbox lets you register applications to get credentials'))
