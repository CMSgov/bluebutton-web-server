...
def match_fhir_id(mbi, hicn_hash, request=None, version=Versions.NOT_AN_API_VERSION) -> MatchFhirIdResult:
    ...
    if mbi:
        try:
            ...
            return MatchFhirIdResult(
                error=str(err.detail), error_type=MatchFhirIdErrorType.UPSTREAM, lookup_type=MatchFhirIdLookupType.MBI
            )

        if fhir_id:
            # Found beneficiary!
            log_match_fhir_id(request, version, fhir_id, hicn_hash, True, 'M', 'FOUND beneficiary via user_mbi')
            return MatchFhirIdResult(fhir_id=fhir_id, lookup_type=MatchFhirIdLookupType.MBI)
    ...
    if hicn_hash and version != Versions.V3:
        ...
        if fhir_id:
            log_match_fhir_id(request, version, fhir_id, hicn_hash, True, 'H', 'FOUND beneficiary via hicn_hash')
            return MatchFhirIdResult(fhir_id=fhir_id, lookup_type=MatchFhirIdLookupType.HICN_HASH)
...
def _validate_patient_search_result(bundle_of_patients):
    ...
    if bundle_of_patients:
        if bundle_of_patients.get('resourceType') == 'Bundle':
                    ...
                    if pt:
                        ...
                        if rs_type == 'Patient':
                            if fhir_id:
                                ...
                                err_detail = (
                                    'Unexpected in Patient search: malformed Patient resource'
                                    '- missing id attribute or id value empty.'
                                )
                        else:
                            err_detail = (
                                "Unexpected in Patient search: expect Patient resource, got 'resourceType': {}."
                            ).format(rs_type)
                    else:
            ...
            err_detail = ("Unexpected in Patient search: expected result of 'resourceType': Bundle, got {}").format(
                bundle_of_patients.get('resourceType')
            )
    else:
