...
from apps.fhir.core.utils import (kickout_404,
                                  kickout_403,
                                  kickout_400,
                                  kickout_500)
...
def create(request, resource_type):
    ...
    try:
        ...
        if interaction_type not in rt.get_supported_interaction_types() and \
                        request.method == 'GET':
            ...
            msg = 'The interaction %s is not permitted on %s FHIR resources ' \
                  'on this FHIR sever.' % (interaction_type, resource_type)
            return kickout_403(msg)

    except SupportedResourceType.DoesNotExist:
        msg = '%s is not a supported resource type on this ' \
              'FHIR server.' % (resource_type)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if isinstance(j, dict):
                kickout_400('The request body did not contain '
                            'a JSON object i.e. {}.')
        except:
        ...
        if 'id' in j:
            ...
            return kickout_400('Create cannot have an id. Perhaps you meant '
        ...
        try:
            ...
            return kickout_500('The JSON Schema on the server did '
        ...
        if json_schema:
            try: 
                ...
                msg = 'JSON Schema ' \
                      'Conformance Error. %s' % (str(sys.exc_info()[1][0]))
    ...
    if request.method not in ('GET', 'POST'):
        ...
        od['note'] = 'Perform an HTTP POST to this URL with the JSON ' \
