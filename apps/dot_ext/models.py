...
class Endorsement(models.Model):
    ...
    def save(self, commit=True, **kwargs):
        if commit:
            ...
            self.iat = datetime.datetime.fromtimestamp(int(payload["iat"]))\
                .strftime('%Y-%m-%d %H:%M:%S')
            self.exp = datetime.datetime.fromtimestamp(int(payload["exp"]))\
                .strftime('%Y-%m-%d %H:%M:%S')
...
class ExpiresInManager(models.Manager):
    ...
    def make_key(client_id, user_id):
        ...
        # Python 3 - avoid TypeError: Unicode-objects must
    ...
    def set_expires_in(self, client_id, user_id, expires_in):
        ...
        instance, _ = self.update_or_create(key=key,
