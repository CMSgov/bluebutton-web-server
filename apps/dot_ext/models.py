...
class AuthFlowUuid(models.Model):
    """
    An instance used to connect the beneficiary authorization flow
    auth_uuid to the resulting access token that is granted.

    This is used for logging via the apps.logging.signals.handle_token_created()
    signal handler.

    Fields:

    code - (Primary Key) The authorization code generated by the authorization server
    auth_uuid - The beneficiary authorization flow tracing UUID
    state - The state provided by the authorize-instance POST request.
    """
    code = models.CharField(primary_key=True, max_length=255, unique=True)  # code comes from oauthlib
    auth_uuid = models.UUIDField(null=True, editable=False)
    state = models.CharField(default='', null=True, max_length=64)
