...
def generate_authflowuuid_records(start_date_str, sample_size):
    ...
    end_t = time.time()
    print(
        'start date = {}, end_date = {}, insert_delete = {}, insert_only = {}, time elapsed (sec) = {}'.format(
            start_date, the_date, insert_delete, insert_only, end_t - start_t
        )
    )
...
class Command(BaseCommand):
    help = (
        'Generate records (as specified by --count) for table dot_ext_authflowuuid: '
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '-s', '--start-date', help="Start time stamp of the auth flow uuid records yyyy-mm-dd, e.g. '2020-11-12'"
        )
        parser.add_argument(
            '-c',
            '--count',
            type=int,
            default=30000,
            help='Total number of auth flow uuid records inserted and then deleted, inserted and remained in the table',
        )
