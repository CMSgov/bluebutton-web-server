...
def delete_authflow_uuid_table(age, limit_on_delete, simulation):
    ...
    delete_sql = DELETE_WITH_LIMIT.format(
        table_name=TARGET_TABLE if not simulation else TARGET_TABLE_COPY,
        age_date=days_before.strftime('%m/%d/%Y'),
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '-a', '--age', type=int, default=30, help='Age (in days) of authflow uuid records qualified for deletion.'
        )
        parser.add_argument(
            '-l', '--limit', type=int, default=30000, help='Limit on number of delete allowed for one run.'
        )
        parser.add_argument(
            '-s',
            '--simulate',
            action='store_true',
            help="Perform delete on 'AuthFlowUuidCopy' (table: dot_ext_authflowuuidcopy) - for local simulation.",
        )
