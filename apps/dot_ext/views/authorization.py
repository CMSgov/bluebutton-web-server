...
class AuthorizationView(DotAuthorizationView):
        ...
        try:
            ...
            if flag.id is None or flag.is_active_for_user(application_user):
                ...
                raise AccessDeniedError(
                    description=APPLICATION_DOES_NOT_HAVE_V3_ENABLED_YET.format(self.application.name)
                )
    ...
    def form_valid(self, form):
        ...
        share_samhsa_value = None
        if self.version == Versions.V3:
            user_approves_sharing_samhsa_data = form.cleaned_data.get('share_samhsa_data')
            if application.part_d_eob_only:
                user_approves_sharing_samhsa_data = True
            share_samhsa_value = 'True' if user_approves_sharing_samhsa_data else 'False'
            set_session_auth_flow_trace_value(self.request, 'auth_share_samhsa_data', share_samhsa_value)
        ...
        try:
            ...
            beneficiary_authorized_application.send(
                ...
                share_samhsa_data=share_samhsa_value,
                scopes=scopes,
                allow=allow,
...
class TokenView(DotTokenView):
    ...
    def _create_or_retrieve_user(self, mbi: str, fhir_id_v3: str, request: HttpRequest = None) -> User:
        ...
        try:
            ...
            # want to confirm that user is an ANB
