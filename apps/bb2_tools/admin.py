...
def gen_ctx_grp_by_flds(clazz_model, grp_flds, uniq_fld, marked_fld_name, marked_fld_val):
    row_cnts_grp_by = clazz_model.objects.all().values(*grp_flds).annotate(row_cnt=Count(uniq_fld)).order_by('row_cnt')
...
def get_my_tokens_widget(u, id):
    ...
    token_viewers = ['mmyaccesstokenviewer', 'myrefreshtokenviewer', 'myarchivedtokenviewer']
...
class TokenCountByAppsAdmin(ReadOnlyAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if clazz_model == AccessToken or clazz_model == ArchivedToken:
            ...
            token_no_demo_total = clazz_model.objects.filter(~Q(scope__icontains='patient/Patient.read')).count()
...
class ApplicationStatsAdmin(ReadOnlyAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        center_panel.append(
            {
                'body': gen_ctx_grp_by_flds(clazz_model, ['active'], 'name', 'active', False),
...
class UserCountByCreateDateAdmin(ReadOnlyAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        top_panel = {
            'type': 'bar-chart',
            'title': ('Beneficiaries Counts by Joined Date, User Total: {}, "BEN": {}, "DEV": {}').format(
                total, total_ben, total_dev
            ),
            'highlight': 'no',
        ...
        center_panel = {
            'type': 'bar-chart',
            'title': ('Developer User Counts by Joined Date: User Total: {}, "BEN": {}, "DEV": {}').format(
                total, total_ben, total_dev
            ),
            'highlight': 'yes',
...
class ConnectedBeneficiaryCountByAppsAdmin(TokenCountByAppsAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        response.context_data['page_desc'] = {
            ...
            'bar_chart_no_demo_title': ('Access Token Count (No Demo Scope) by Apps, Total ({})').format(
