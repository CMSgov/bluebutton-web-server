...
/* Set the report_date_range to be selected.
 ...
 bb2.prod_global_state_per_app table.
...
WITH report_date_range AS (
  /* Select gets the MIN report_date to select new records from 
   the bb2.events_prod_perf_mon table */
  /*
  ...
  */
  /*
   ...
   bb2.prod_global_state_per_app table via the select below: */
  SELECT
    (
      SELECT
        date_trunc('week', MAX(report_date))
      FROM
        bb2.prod_global_state_per_app
    ) AS min_report_date,
