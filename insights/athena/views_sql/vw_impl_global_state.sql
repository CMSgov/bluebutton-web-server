...
(
    select time_of_event, path, fhir_id, req_qparam_lastupdated from "bb2"."events_impl_perf_mon"
    where
    (
...
(
    select time_of_event, auth_require_demographic_scopes, auth_crosswalk_action, auth_share_demographic_scopes,
           ...
           json_extract(user, '$.crosswalk.fhir_id') as fhir_id from "bb2"."events_impl_perf_mon"
    where
    (
        type = 'Authorization'
        and vpc = 'impl'
