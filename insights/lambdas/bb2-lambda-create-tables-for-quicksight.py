...
def lambda_handler(event, context):
    ...
    # Drop table 
    params["query"] = ("DROP TABLE IF EXISTS " + event["DATABASE"] + "." + event["ENV"]
             + "_global_state")
   
    drop_output_filename = athena_to_s3_complete(session, params)
   
    # Create table
    params["query"] = ("CREATE TABLE IF NOT EXISTS " + event["DATABASE"] + "." + event["ENV"]
             + "_global_state AS SELECT * FROM " + event["DATABASE"] + ".vw_"
             + event["ENV"] + "_global_state")
